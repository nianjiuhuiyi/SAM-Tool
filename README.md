...
16. 添加打开文件夹功能，方便分文件夹标注时快速打开，而不用关闭程序，去修改变量后再命令行启动。
...
- [ ] TODO: bug, 在一个文件夹标注好后，用左上角打开新文件夹标注，右边的类别显示已经选了，此时去标注会提示没选择类别，需要手动再去点击类别，如果显示的类别是自己想要的类别，就用先点击别的类别再点回来。
- [ ] TODO: bug, 当标注的目标过大，标注时，目标的分割同时接触到了图片边缘的两边或以上，在点击“添加”确定标注后会出现很奇怪的分割结果，这对目标检测没影响，但对目标分割是不对的。
